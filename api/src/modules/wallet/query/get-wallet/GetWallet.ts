...
import { CDBReturnCalculator } from "../../../cdb/domain/cdb-return-calculator/CDBReturnCalculator";
...
export type GetWalletRequestModel = {
	investorID: InvestorIDType;
};
export type GetWalletResponseModel = (Omit<
	PooledWalletAsset,
	"assetID" | "currency" | "lastPrice"
> & {
	walletPercentage: number;
})[];
export class GetWallet
	implements Query<GetWalletRequestModel, GetWalletResponseModel>
{
	constructor(
	...
	async get({
		investorID,
	}: GetWalletRequestModel): Promise<GetWalletResponseModel> {
		...
		const total =
			...
			brlBalance +
			usdBalance * 5;
		return pooledWalletAssets.map(
			({ assetID, currency, lastPrice, walletValueAmount, ...data }) => ({
				...data,
				walletValueAmount,
				walletPercentage: Math.round((walletValueAmount / total) * 100) / 100,
			}),
		);
		/* return Promise.all(
			pooledWalletAssets.map(({ assetID, currency, lastPrice, ...data }) => ({
				...data,
			})),
		); */
	}
